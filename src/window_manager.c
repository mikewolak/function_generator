...
static gboolean do_device_switch_idle(gpointer user_data) {
    ...
    // Need to disable audio before switching
    if (data->manager->generator) {
        waveform_generator_set_audio_enabled(data->manager->generator, false);
    }
    
    audio_manager_switch_device(data->manager->audio_manager, data->device_name);
...
static void on_device_selected(GtkMenuItem *item, gpointer user_data) {
    ...
    if (device_name && manager->audio_manager) {
        ...
        data->manager = manager;  // Store the whole manager
